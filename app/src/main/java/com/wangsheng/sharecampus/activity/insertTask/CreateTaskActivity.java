package com.wangsheng.sharecampus.activity.insertTask;
...
public class CreateTaskActivity extends AppCompatActivity{
    ...
    private void createTask(){
        ...
        CreateTaskBean task = new CreateTaskBean();
        task.setPublisherId(Integer.getInteger(SharedUtil.getParam("userId","").toString()));
        task.setCategory(1);
        task.setCounts(Integer.getInteger(peopletext.getText().toString().split("人")[0]));
        task.setDescription(descriptiontext.getText().toString());
        task.setTitle(titletext.getText().toString());
        task.setPrice(Integer.getInteger(pricetext.getText().toString().split("金")[0]));
        task.setStarttime(DateUtil.getDate());
        task.setEndtime(timetext.getText().toString().split("天")[0]);
        Observable<ResponseInfo<CreateTaskBean>> call = taskService.insert(task);
        call.compose(RxSchedulersHelper.<ResponseInfo<CreateTaskBean>>io_main())
                .subscribe(new HttpObserver<CreateTaskBean>() {
                    @Override
                    public void onSuccess(CreateTaskBean taskBean) {
